...
@st.cache_resource
def carregar_modelo():
    pacote = joblib.load("modelo_obesidade.pkl")
    return pacote
...
st.write(
    ...
    "do nível de obesidade com base em características físicas e hábitos de vida."
)
...
with aba1:
    ...
    if st.button("Realizar predição"):
        ...
        st.info(
            "Esta predição é um apoio analítico e não substitui a avaliação médica profissional."
        )
...
with aba2:
    ...
    grafico_obesidade = px.histogram(
        df,
        x="Obesity_Descricao",
    ...
    grafico_obesidade.update_layout(
        xaxis_title="Nível de obesidade",
        yaxis_title="Quantidade",
        showlegend=False
    )

    st.plotly_chart(grafico_obesidade, use_container_width=True)
    ...
    with col_b:
        st.subheader("Histórico familiar e obesidade")

        grafico_historico = px.histogram(
            df,
            x="family_history",
            color="Grupo_Obesidade",
            barmode="group",
...
with aba3:
    ...
    O objetivo do modelo é classificar o nível de obesidade em uma das seguintes categorias:
    """)
